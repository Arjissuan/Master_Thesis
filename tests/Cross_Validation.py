    ...
    def cross_validation(self):
        ...
        for j, clss_set in enumerate(self.cross_val_data):
            ...
            for i, splitted_data in enumerate(clss_set):
                ...
                mlp_pred = self.__mlp.predict(X=test_x)
                rfc_pred = self.__rfc.predict(X=test_x)
                svc_pred = self.__svc.predict(X=test_x)
                bayes_pred = self.__bayes.predict(X=test_x)
